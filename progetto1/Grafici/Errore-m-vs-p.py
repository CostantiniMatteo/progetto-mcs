...
plt.plot(x, y_py, color=tableau20[8], dashes=[2, 2], marker='o', label='Media Python')

y_mat = [sum(y) / 2 for y in zip(y_mat_u, y_mat_w)]
plt.plot(x, y_mat, color=tableau20[16], dashes=[2, 2], marker='o', label='Media Matlab')
...
plt.savefig('immagini/Erore-m-vs-p_dim.png')
plt.show()
