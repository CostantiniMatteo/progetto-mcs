...
y_py_w = dt.loc[(dt['os'] == 'windows') & (dt['lang'] == 'python'),'memory']
y_py_u = dt.loc[(dt['os'] == 'ubuntu') & (dt['lang'] == 'python'),'memory']

y_mat_u = dt.loc[(dt['os'] == 'ubuntu') & (dt['lang'] == 'matlab'),'memory']
y_mat_w = dt.loc[(dt['os'] == 'windows') & (dt['lang'] == 'matlab'),'memory']

y_py = [sum(y) / 2 for y in zip(y_py_u, y_py_w)]
plt.plot(x, y_py, color=tableau20[4], dashes=[2, 2], marker='o', label='Media Python')

y_mat = [sum(y) / 2 for y in zip(y_mat_u, y_mat_w)]
plt.plot(x, y_mat, color=tableau20[6], dashes=[2, 2], marker='o', label='Media Matlab')
...
plt.savefig('immagini/Memoria-m-vs-p_dim.png')
plt.show()
